...
function ToolFallbackContent({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "flex flex-col gap-1 px-2 pt-1 pb-1.5 ease-[cubic-bezier(0.32,0.72,0,1)] motion-reduce:animate-none",
...
const formatJsonOrText = (value: string) => {
  try {
...
function ToolFallbackArgs({
  ...
  return (
    ...
    >
      ...
      <pre className="aui-tool-fallback-args-value bg-muted/50 text-foreground/90 overflow-x-auto rounded p-2 text-[11px] leading-4 whitespace-pre-wrap">
        {formatJsonOrText(argsText)}
      </pre>
    </div>
  );
}
...
function ToolFallbackResult({
  ...
  return (
    ...
    >
      ...
      <pre className="aui-tool-fallback-result-content bg-muted/50 text-foreground/90 mt-1 overflow-x-auto rounded p-2 text-[11px] leading-4 whitespace-pre-wrap">
        {typeof result === "string"
          ? formatJsonOrText(result)
...
const toolNameFromArgs = (args: Record<string, unknown>) => {
  if (typeof args.tool !== "string") return null;
  return typeof args.server === "string" ? `${args.server} · ${args.tool}` : args.tool;
};

const summarizeToolArgs = (argsText?: string) => {
  if (!argsText) return null;

  try {
    const args = JSON.parse(argsText) as Record<string, unknown>;
    const primary =
      toolNameFromArgs(args) ?? args.path ?? args.command ?? args.filePath ?? args.url;
...
const ToolFallbackImpl: ToolCallMessagePartComponent = ({
  ...
  const statusType = status?.type ?? "complete";
  const Icon = statusIconMap[statusType];
  const summary = summarizeToolArgs(argsText);
  ...
  return (
    <ToolFallbackRoot
      ...
      className={cn(
        "my-0.5 rounded-md border border-app-line bg-app-panel/60 text-xs",
        isCancelled && "opacity-60",
      )}
    >
      <CollapsibleTrigger className="flex w-full min-w-0 items-center gap-1.5 px-2 py-1 text-left hover:bg-app-surface/60">
        <Icon
          className={cn(
            "size-3.5 shrink-0 text-muted-foreground",
            statusType === "running" && "animate-spin [animation-duration:0.6s]",
            statusType === "incomplete" && "text-red-400",
          )}
        />
        <span className="shrink-0 font-medium text-foreground">{toolName}</span>
        {summary && (
          <code className="min-w-0 truncate rounded bg-muted/60 px-1 py-0.5 text-[11px] leading-4 text-foreground/80">
            {summary}
          </code>
        )}
        <ToolFallbackDuration className="ml-auto" />
        <ChevronDownIcon
          className={cn(
